# 第13章：三种 Transport 横向对比：NIO vs Epoll vs io_uring

> **本章目标**：在已经深入学习了 NIO（第3章）、Epoll（第11章）、io_uring（第12章）之后，本章做一次系统性的横向对比，回答三个核心问题：
...
<!-- 核对记录：已对照 NioIoHandler.java run() 方法，差异：无 -->
...
<!-- 核对记录：已对照 NioIoHandler.java openSelector() 方法，差异：无 -->
...
### 3.3 Epoll ET 模式的读写循环

ET 模式下，Netty 必须循环读/写直到 EAGAIN，否则数据会丢失：

```java
// AbstractEpollStreamChannel.EpollStreamUnsafe.epollInReady() 读循环（ET 模式）
do {
    byteBuf = allocHandle.allocate(allocator);
    allocHandle.lastBytesRead(doReadBytes(byteBuf));
    if (allocHandle.lastBytesRead() <= 0) {
...
**ET vs LT 的性能差异**：
- LT：每次 `epoll_wait` 只要 fd 有数据就上报，即使上次没读完也会再次通知
- ET：只在数据到达时通知一次，必须一次性读完，减少 `epoll_wait` 的唤醒次数
...
<!-- 核对记录：已对照 NioIoHandler.java 全量源码（814行），run()、openSelector()、processSelectedKeys()、select() 方法逐行核对，差异：无 -->
