# 第24章：框架复用 —— Dubbo / gRPC / WebFlux 如何站在 Netty 肩膀上
...
```
┌─────────────────────────────────────────────────────────────┐
│                    上层框架（Dubbo / gRPC / WebFlux）          │
├─────────────────────────────────────────────────────────────┤
│  ChannelHandler / Pipeline  ← 业务逻辑插拔点（框架扩展入口）    │
├─────────────────────────────────────────────────────────────┤
│  编解码基础设施  ← LengthFieldBasedFrameDecoder / SslHandler   │
├─────────────────────────────────────────────────────────────┤
│  ByteBuf + 内存池  ← 零 GC 压力的高性能缓冲区                   │
├─────────────────────────────────────────────────────────────┤
│  Future / Promise  ← 统一的异步编程模型                         │
├─────────────────────────────────────────────────────────────┤
│  EventLoop + IoHandler  ← 线程模型与 I/O 多路复用               │
├─────────────────────────────────────────────────────────────┤
│  NIO / Epoll / io_uring  ← 可插拔的传输层实现                   │
└─────────────────────────────────────────────────────────────┘
```
...
<!-- 核对记录：已对照 Channel.java 第77-203行，差异：无 -->
...
<!-- 核对记录：已对照 ChannelHandler.java 第178-216行，差异：无 -->
...
<!-- 核对记录：已对照 ChannelInboundHandlerAdapter.java 第34行、ChannelDuplexHandler.java 第29行、ChannelOutboundHandlerAdapter.java 第26行、SimpleChannelInboundHandler.java 第42行，差异：无 -->
...
<!-- 核对记录：已对照 IoHandler.java 第33-84行，差异：无 -->
...
<!-- 核对记录：已对照 NioIoHandler.java 第776行、EpollIoHandler.java 第100行、MultiThreadIoEventLoopGroup.java 第53行，差异：无 -->
...
<!-- 核对记录：已对照 ChannelInitializer.java 第54行（类声明）、第70行（initChannel 抽象方法），差异：无 -->
...
<!-- 核对记录：已对照 LengthFieldBasedFrameDecoder.java 第187行、第190-195行，差异：无 -->
...
<!-- 核对记录：已对照 IdleStateHandler.java 第100行、第113-115行，差异：无 -->
...
<!-- 核对记录：已对照 SslHandler.java 第170行、第418-419行、第632-636行，差异：无 -->
...
<!-- 核对记录：已对照 ChannelOption.java 第114行，差异：无 -->
