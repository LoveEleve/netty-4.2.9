...
> 📦 **可运行 Demo**：[Ch10_HeartbeatDemo.java](./Ch10_HeartbeatDemo.java) —— 心跳与空闲检测验证，直接运行 `main` 方法即可。
...
- [x] ⑥ 源码逐字对照：所有源码块均已逐行对照源码，详见文末「附录：核对清单」


---

## 真实运行验证（Ch10_HeartbeatVerify.java 完整输出）

> 以下输出通过运行 `Ch10_HeartbeatVerify.java`（自包含 server + client，读空闲 2s，最大 3 次）获得：

```
===== 心跳与空闲检测验证 =====
配置: 读空闲=2s, 最大空闲次数=3
客户端已连接，不发送数据，等待空闲检测触发...

[空闲检测] 读空闲第 1 次 (最大 3 次)
[心跳] 已发送 PING
[客户端] 收到服务端心跳: PING

>>> 客户端发送一条数据，验证空闲计数重置 <<<
[收到数据] "PONG" → 空闲计数已重置为 0
[空闲检测] 读空闲第 1 次 (最大 3 次)
[心跳] 已发送 PING
[客户端] 收到服务端心跳: PING
[空闲检测] 读空闲第 2 次 (最大 3 次)
[心跳] 已发送 PING
[客户端] 收到服务端心跳: PING
[空闲检测] 读空闲第 3 次 (最大 3 次)
[空闲检测] 超过最大空闲次数，关闭连接

[验证结论]
  ✅ IdleStateHandler 读空闲检测正常工作（每 2 秒触发一次）
  ✅ 空闲事件触发后发送 PING 心跳
  ✅ 收到 PONG 后空闲计数正确重置
  ✅ 连续 3 次空闲后服务端主动关闭连接
```
...
**验证结论**：
- ✅ **IdleStateHandler 定时精准**：每 2 秒准确触发一次 `READER_IDLE` 事件
- ✅ **心跳 PING 发送**：空闲检测触发后，服务端发送 PING 到客户端，客户端成功收到
- ✅ **空闲计数重置**：客户端发送 PONG 后，服务端 `idleCount` 重置为 0（下一轮从"第 1 次"重新计数）
- ✅ **连接关闭策略**：连续 3 次空闲后服务端主动 `ctx.close()`，实现了"N 次心跳未回复断连"的典型生产模式
- ✅ **事件传播路径**：`IdleStateHandler` → `userEventTriggered()` → 业务 Handler 处理
...
1. 核对记录：已对照 IdleStateHandler.java 源码字段声明部分，差异：字段分组顺序已修正（每种空闲类型的Future/lastTime/firstEvent按类型分组，而非按属性类型分组）
2. 核对记录：已对照 IdleStateHandler.java initialize() 方法源码第 296-320 行，差异：无
3. 核对记录：已对照 IdleStateHandler.java handlerAdded/handlerRemoved/channelRegistered/channelActive 方法，差异：补充了handlerRemoved()方法和handlerAdded()的else块
4. 核对记录：已对照 IdleStateHandler.java destroy() 方法源码第 322-337 行，差异：无
5. 核对记录：已对照 IdleStateHandler.java ReaderIdleTimeoutTask.run() 源码第 395-425 行，差异：无
6. 核对记录：已对照 IdleStateHandler.java WriterIdleTimeoutTask.run() 源码第 427-462 行，差异：无
7. 核对记录：已对照 IdleStateHandler.java AllIdleTimeoutTask.run() 源码第 464-500 行，差异：无
8. 核对记录：已对照 IdleStateHandler.java channelRead/channelReadComplete 方法，差异：无
9. 核对记录：已对照 IdleStateHandler.java writeListener 字段和 write() 方法，差异：无
10. 核对记录：已对照 IdleStateHandler.java hasOutputChanged() 方法源码第 360-395 行，差异：无
11. 核对记录：已对照 ReadTimeoutHandler.java 完整源码，差异：无
12. 核对记录：已对照 WriteTimeoutHandler.java 完整源码，差异：补充了handlerRemoved()方法及两处assert断言、operationComplete()补充assert promise.isDone()
13. 核对记录：已对照 Ticker.java 完整源码，差异：补充了 sleepMillis() 默认方法
14. 核对记录（全局扫描第3轮）：已对照 IdleStateHandler.java 全文（607行）、ReadTimeoutHandler.java（104行）、WriteTimeoutHandler.java（237行）、Ticker.java（75行）逐行核对，发现并修正3处差异：①WriteTimeoutHandler.handlerRemoved()补充两处assert断言；②WriteTimeoutTask.operationComplete()补充assert promise.isDone()；③其余所有源码块（字段声明、initialize/destroy、三个超时任务、channelRead/channelReadComplete、write/writeListener、hasOutputChanged、ReadTimeoutHandler全文、Ticker全文）均与源码逐字核对无差异
