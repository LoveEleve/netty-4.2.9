# 12 - Native Transport：Epoll 深度解析

> **本章目标**：搞清楚 Netty Epoll Transport 的完整架构——从 JNI 加载到 epoll_wait 循环，从 Channel 注册到 LT/ET 模式选择，从 4.2 新架构（IoHandler SPI）到与 NIO 的本质差异。读完后能回答：为什么 Epoll 比 NIO 快？Channel fd 用 LT 模式有什么好处？4.2 的 EpollIoHandler 和旧版 EpollEventLoop 有什么区别？
...
核心收益：
1. **绕过 JDK 抽象层**：JNI 直接调用 `epoll_wait`/`read`/`writev`，省去 JDK NIO 的 `SocketChannel` → `IOUtil` 层层封装（注：Channel fd 使用 LT 模式，`EpollMode` 已 `@Deprecated`；仅 eventFd/timerFd 使用 ET）
...
**Q1：Epoll 相比 NIO Selector 的核心优势是什么？** 🔥🔥🔥

**A**：三个核心优势：
1. **O(1) 就绪检测**：NIO 的 `Selector.select()` 每次都需要把 fd 集合从用户态拷贝到内核态（`O(n)` 拷贝），epoll 使用 `epoll_ctl` 增量注册，`epoll_wait` 只返回就绪的 fd（无需遍历所有 fd）。
2. **边缘触发（ET）**：NIO 只支持水平触发（LT），每次 `select` 都会报告所有可读 fd；Epoll 的 ET 模式只在状态变化时通知一次，减少系统调用次数。Netty 默认使用 ET 模式（`EpollMode.EDGE_TRIGGERED`）。
3. **减少 JNI 开销**：NIO Selector 底层有 `fdToKey` HashMap 映射 + `selectedKeys` Set 操作，Netty 替换为数组后仍有 JNI 边界；Epoll 直接操作堆外内存（`EpollEventArray`），减少 Java ↔ Native 数据转换。

---

**Q2：Netty 的 Epoll 实现为什么要自己写 JNI，不直接用 JDK 的 EPollSelectorImpl？** 🔥🔥

**A**：
- JDK 的 EPollSelectorImpl **固定使用水平触发（LT）**，无法切换为边缘触发
- JDK 无法暴露 `epoll_ctl` 的增量操作，每次 `select()` 仍有 `O(n)` 开销
- JDK 不支持 Linux 特有的 socket 选项（如 `TCP_CORK`、`TCP_QUICKACK`、`SO_REUSEPORT`）
- Netty 使用 `eventfd` + `timerfd` 替代 JDK 的 `pipe` 唤醒机制，减少 fd 开销和系统调用

---

**Q3：`EpollIoHandler.run()` 中 `epoll_wait` 的超时时间是如何计算的？** 🔥

**A**：超时时间取决于**最近的定时任务**：
1. 取 `scheduledTaskQueue` 中最近任务的 `deadlineNanos`
2. 如果 `deadlineNanos` 在 `timerfd` 可表示范围内（<999999999ns），使用 `timerfd_settime` 精确定时，`epoll_wait` 传 `-1`（无限等待，由 timerfd 触发返回）
3. 如果超出 `timerfd` 范围，将 `timerFd` 设为最大值，计算剩余差值作为 `epoll_wait` 的毫秒超时参数
4. 如果没有定时任务，`epoll_wait` 传 `-1` 永久阻塞直到 IO 事件或 `eventfd` 唤醒

---

**Q4：`eventfd` 和 `timerfd` 分别用来做什么？为什么不用一个 `pipe` 统一替代？** 🔥🔥

**A**：
- **eventfd**：用于线程间唤醒（外部线程提交任务时唤醒 `epoll_wait`），只需一个 fd（JDK 的 `pipe` 需要两个 fd）
- **timerfd**：用于精确定时（精度到纳秒），让 `epoll_wait` 在定时任务到期时返回，**无需轮询**

不统一的原因：eventfd 是**事件通知**（写 1 个 uint64），timerfd 是**定时器**（内核自动触发），语义完全不同。如果用 pipe 替代 timerfd，需要额外的线程或超时来实现定时，增加复杂度。

---

**Q5：`EpollEventArray` 为什么直接操作堆外内存（Unsafe），不用 `ByteBuffer.allocateDirect()`？** 🔥

**A**：
- `epoll_wait` 需要的参数是 `struct epoll_event*`（C 语言指针），直接传堆外内存地址最高效
- `Unsafe.getInt(address + offset)` 直接读取内存，不经过任何边界检查，比 `ByteBuffer.getInt()` 快
- 扩容时翻倍重新分配 + 释放旧内存，简单高效

---

**Q6：Netty 的 Epoll 边缘触发（ET）模式下，如何保证不丢数据？** 🔥🔥🔥

**A**：ET 模式只在状态变化时通知一次，所以 **必须一次性读完所有数据**。Netty 的处理方式：
1. `EpollRecvByteAllocatorHandle` 循环读取，直到 `read()` 返回 0 或负值
2. 每次读取后检查 `maybeMoreDataToRead`（如果读满了 allocator 分配的缓冲区，可能还有数据）
3. 如果 `readPending=true` 且可能有更多数据，会再次触发 `epollIn()` 事件
4. `epollRdHup()` 处理对端关闭，确保 FIN 不被遗漏

---

**Q7：`registrations` 为什么用 `IntObjectMap`（基于数组）而不是 `HashMap`？** 🔥

**A**：因为 fd 是 int 类型，`IntObjectHashMap` 基于开放寻址法，key 直接存储为 `int`（不需要装箱为 `Integer`），相比 `HashMap<Integer, Registration>`：
- 无装箱开销（`Integer.valueOf()` 和 GC 压力）
- 无 `Entry` 对象开销（`HashMap.Node` 每个 32 字节）
- 内存更紧凑，CPU 缓存友好
- 时间复杂度相同（O(1) 均摊），但常数因子更小

---

**Q8：`pendingWakeup` 标志有什么作用？为什么不在 `processReady` 处理 eventfd 时直接读取？** 🔥

**A**：`pendingWakeup` 是一个**延迟读取优化**。当 `epoll_wait` 返回且 `eventfd` 就绪时，不立即调用 `eventFdRead()`（一次系统调用），而是设置 `pendingWakeup=true`，在下一次 `epoll_wait` 之前再读取。这样可以减少一次系统调用。但如果下一次 `epoll_wait` 的超时是 `-1`（无限等待），必须在进入等待前读取 eventfd 并清除 `pendingWakeup`，否则 eventfd 一直就绪，`epoll_wait` 会立即返回（空轮询）。
...
- [x] ⑥ 源码逐字对照：所有源码块均已通过工具调用核对，详见文末「附录：核对清单」
...
1. 核对记录：已对照 EpollIoHandler.java 字段声明（第55-85行）、AbstractEpollChannel.java 字段声明（第62-75行），差异：无
2. 核对记录：已对照 Epoll.java 完整源码（128行），差异：无
3. 核对记录：已对照 EpollIoHandler.java 字段声明（第55-90行），差异：无
4. 核对记录：已对照 EpollIoHandler.java openFileDescriptors() 方法（第130-175行），差异：无
5. 核对记录：已对照 EpollIoHandler.java register() 方法（第270-295行），差异：无
6. 核对记录：已对照 EpollIoHandler.java DefaultEpollIoRegistration 内部类（第195-265行），差异：补充了attachment()方法和close()方法
7. 核对记录：已对照 EpollIoHandler.java run() 方法（第340-430行），差异：无
8. 核对记录：已对照 EpollIoHandler.java processReady() 方法（第460-495行），差异：无
9. 核对记录：已对照 EpollIoOps.java 完整源码（210行），差异：无
10. 核对记录：已对照 EpollMode.java 完整源码（40行），差异：无
11. 核对记录：已对照 AbstractEpollChannel.java 字段声明（第62-75行），差异：无
12. 核对记录：已对照 AbstractEpollChannel.java doRegister() 方法（第230-245行），差异：无
13. 核对记录：已对照 AbstractEpollChannel.java doBeginRead() 方法（第200-210行），差异：无
14. 核对记录：已对照 AbstractEpollChannel.java AbstractEpollUnsafe.handle() 方法（第490-530行），差异：无
15. 核对记录：已对照 EpollServerSocketChannel.java doBind() 方法（第80-92行），差异：无
16. 核对记录：已对照 AbstractEpollServerChannel.java EpollServerSocketUnsafe.epollInReady() 方法（第80-130行），差异：无
17. 核对记录：已对照 EpollIoHandler.java wakeup() 方法（第185-190行），差异：无
18. 核对记录：已对照 EpollEventArray.java 完整源码（128行），差异：无
19. 核对记录（全局扫描第3轮）：已对照 EpollIoHandler.java（576行）、AbstractEpollChannel.java（850行）、Epoll.java（128行）、EpollIoOps.java（210行）逐字核对，发现并修正3处差异：①Epoll.java静态初始化块补充日志打印段（cause!=null时打印debug/trace日志）；②DefaultEpollIoRegistration补充attachment()方法（返回nativeArrays）；③DefaultEpollIoRegistration补充close()方法（cancel+handle.close()）。其余所有源码块均与源码逐字核对无差异
