...
<!-- 核对记录：已对照 IoHandler.java 全文（86行）、IoHandlerFactory.java（45行）、IoHandlerContext.java（84行）、IoHandle.java（61行）、IoRegistration.java（54行）、IoEvent.java（26行）、IoOps.java（27行），差异：无 -->
...
<!-- 核对记录：已对照 SingleThreadIoEventLoop.java 第37-75行字段声明，差异：无 -->
...
<!-- 核对记录：已对照 SingleThreadIoEventLoop.java 第196-213行 run() 方法，差异：无 -->
...
<!-- 核对记录：已对照 SingleThreadIoEventLoop.java 第228-260行 register/registerForIo0 方法，差异：无 -->
...
<!-- 核对记录：已对照 NioIoHandler.java 第55-107行字段声明，差异：无 -->
...
<!-- 核对记录：已对照 NioIoHandler.java 第340-400行 run() 方法，差异：无 -->
...
<!-- 核对记录：已对照 NioIoHandler.java 第270-380行 DefaultNioRegistration 内部类，差异：补充了 isValid() 方法（已修正） -->
...
<!-- 核对记录：已对照 NioIoHandler.java 第770-814行 newFactory() 方法，差异：补充了 ObjectUtil.checkNotNull 参数校验（已修正） -->
...
<!-- 核对记录：已对照 MultiThreadIoEventLoopGroup.java 第40-229行，差异：newChild() 补充了 @SuppressWarnings("unused") 注解（已修正） -->
...
<!-- 核对记录：已对照 AutoScalingEventExecutorChooserFactory.java 第83-92行字段声明，差异：补充了 NO_OOP_TASK 静态字段（已修正） -->
...
<!-- 核对记录：已对照 AutoScalingEventExecutorChooserFactory.java 第148-163行 AutoScalingState 内部类，差异：无 -->
...
<!-- 核对记录：已对照 AutoScalingEventExecutorChooserFactory.java 第270-390行 UtilizationMonitor.run() 方法，差异：无 -->
...
<!-- 核对记录：已对照 AutoScalingEventExecutorChooserFactory.java 第195-245行 tryScaleUpBy() 方法，差异：无 -->
...
<!-- 核对记录：已对照 DefaultEventExecutorChooserFactory.java 全文（74行），差异：无 -->
