...
> 📦 **可运行 Demo**：[Ch06_ByteBufDemo.java](./Ch06_ByteBufDemo.java) —— ByteBuf 基础操作与内存池验证，直接运行 `main` 方法即可。
...
> ℹ️ 以上 3 个方案需修改 Netty 源码添加打印才能看到输出，输出内容基于源码逻辑推理。以下通过 `Ch06_ByteBufDemo.java` 提供不修改源码的真实运行输出。

### 真实运行验证（Ch06_ByteBufDemo.java 完整输出）

> 以下输出通过直接运行 `Ch06_ByteBufDemo.java` 的 `main` 方法获得（OpenJDK 11，Linux x86_64）：

```
===== 1. 双指针读写分离 =====
初始状态: readerIndex=0 writerIndex=0 capacity=256
写入后:   readerIndex=0 writerIndex=13 readableBytes=13
读5字节后: readerIndex=5 读到: Hello
discard后: readerIndex=0 writerIndex=8

===== 2. 堆内 vs 直接内存 =====
Heap ByteBuf: hasArray=true class=InstrumentedUnpooledUnsafeHeapByteBuf
Direct ByteBuf: hasArray=false class=InstrumentedUnpooledUnsafeDirectByteBuf
...
release1次后 refCnt=1
release2次后 refCnt=0
已释放后写入抛异常: IllegalReferenceCountException
...
===== 5. 池化分配器分配/释放 =====
池化 direct 分配 100000 次耗时: 58.45 ms
非池化 direct 分配 100000 次耗时: 43.07 ms
池化快了 0.7 倍
提示：在 EventLoop 线程中，PoolThreadCache 命中率极高，池化优势更加显著

✅ Demo 结束
```

**验证结论**：
- ✅ **双指针读写分离**：写入 "Hello, Netty!"(13字节) 后 writerIndex=13，读 5 字节后 readerIndex=5，discardReadBytes 后 readerIndex 回 0
- ✅ **堆内 vs 直接内存**：堆内 ByteBuf 有 backing array（`hasArray=true`），直接内存没有（`hasArray=false`）
- ✅ **引用计数**：`retain()` 加 1，`release()` 减 1，减到 0 后操作抛 `IllegalReferenceCountException`
- ✅ **slice 零拷贝**：修改 slice 后原始 ByteBuf 也变化（"ABCDEF" → "AXCDEF"），证明共享底层内存
- ⚠️ **池化 vs 非池化**：在单线程非 EventLoop 环境下，256B 小块的池化分配因 Arena/ThreadCache 管理开销未必比非池化快；但在生产环境的 EventLoop 线程中，大量重复分配/释放同一 sizeIdx 时，PoolThreadCache 命中率极高，池化优势非常显著
...
## Ch06_ByteBufVerify 真实运行输出

> 📦 **验证代码**：[Ch06_ByteBufVerify.java](./Ch06_ByteBufVerify.java) — 直接运行 `main` 方法即可复现以下输出。

```
===== Ch06 ByteBuf 与内存池内部机制验证 =====

--- 验证 1：SizeClasses 规格对齐 ---
  请求     1 → 实际容量     1 ⚠ 期望16 (4.2 AdaptiveAllocator 精确分配)
  请求    16 → 实际容量    16 ✅
  请求    17 → 实际容量    17 ⚠ 期望32 (4.2 精确分配)
  请求   100 → 实际容量   100 ⚠ 期望112 (4.2 精确分配)
  请求   500 → 实际容量   500 ⚠ 期望512 (4.2 精确分配)
  请求  1000 → 实际容量  1000 ⚠ 期望1024 (4.2 精确分配)
  请求  4000 → 实际容量  4000 ⚠ 期望4096 (4.2 精确分配)
  请求 10000 → 实际容量 10000 ⚠ 期望12288 (4.2 精确分配)

--- 验证 2：Arena 数量 ---
  CPU 核心数:      16
  堆内 Arena 数:   32 (期望 ≈ 32, 即 2 * nCPU)
  直接内存 Arena 数: 32 (期望 ≈ 32)
  验证: ✅ Arena 数合理

--- 验证 3：ByteBuf 实际类型 ---
  池化+直接:  PooledUnsafeDirectByteBuf
  池化+堆内:  PooledUnsafeHeapByteBuf
  非池化+直接: InstrumentedUnpooledUnsafeDirectByteBuf
  非池化+堆内: InstrumentedUnpooledUnsafeHeapByteBuf
  池化类型包含 'Pooled': ✅ 是

--- 验证 4：PoolChunk 默认大小 ---
  chunkSize = 4194304 bytes (4 MB)
  期望 = 4194304 bytes (4 MB)
  验证: ✅ 通过

--- 验证 5：Subpage 阈值验证 ---
  请求 16    → 容量 16    class=PooledUnsafeHeapByteBuf
  请求 256   → 容量 256   class=PooledUnsafeHeapByteBuf
  请求 4096  → 容量 4096  class=PooledUnsafeHeapByteBuf
  请求 8192  → 容量 8192  class=PooledUnsafeHeapByteBuf
  请求 16384 → 容量 16384 class=PooledUnsafeHeapByteBuf
  Small 容量 < pageSize(8192): ✅ 是
  Normal 容量 >= pageSize(8192): ✅ 是

--- 验证 6：PoolThreadCache 线程缓存命中 ---
  缓存命中场景: 100000 次分配/释放耗时 72.88 ms
  分配数增量: 100000
  平均每次: 729 ns
  ✅ PoolThreadCache 缓存加速验证完成

--- 验证 7：引用计数与池化回收 ---
  第1次分配: class=PooledUnsafeHeapByteBuf refCnt=1
  release 后 refCnt=0 (回收到缓存)
  第2次分配: class=PooledUnsafeHeapByteBuf refCnt=1
  类型一致: ✅ 是（对象被回收复用）
...
> **🔍 验证结论**：
> - SizeClasses 对齐 ⚠️：Netty 4.2 的 `AdaptivePoolingAllocator` 改变了分配策略，实际容量可能精确等于请求大小（而非 4.1 的向上对齐到 sizeClass），SizeClasses 的对齐规则仍在 `PooledByteBufAllocator` 中生效
> - Arena 数量 ✅：`2 * nCPU = 32`
> - ByteBuf 类型 ✅：池化用 `PooledUnsafe*ByteBuf`，非池化用 `InstrumentedUnpooledUnsafe*ByteBuf`
> - PoolChunk 大小 ✅：默认 4MB
> - Subpage 阈值 ✅：< pageSize(8192) 走 Subpage 分配
> - PoolThreadCache ✅：缓存命中时平均 729ns/次，无锁分配
> - 引用计数回收 ✅：release 后对象回池，再分配时类型一致
...
<!-- 核对记录（2026-03-03 源码逐字核对）：
  1. 已对照 AbstractByteBuf 全部字段（6个核心）+ readByte/writeByte/clear/discardReadBytes/ensureWritable0（AbstractByteBuf.java），差异：无
  2. 已对照 AbstractReferenceCountedByteBuf 字段（RefCnt refCnt）（AbstractReferenceCountedByteBuf.java），差异：无
  3. 已对照 PooledByteBuf 全部字段（10个）+ deallocate()（PooledByteBuf.java），差异：无
  4. 已对照 RefCnt 三条路径选择 + refCnt 编码规则（value = refCnt * 2, odd = 已释放）（RefCnt.java），差异：无
  5. 已对照 PooledByteBufAllocator 全部实例字段（9个）+ 默认参数 + newHeapBuffer()（PooledByteBufAllocator.java），差异：无
  6. 已对照 PoolArena.allocate() 三路分配逻辑 + allocateNormal() 分配顺序（PoolArena.java），差异：无
  结论：全部无差异
-->
