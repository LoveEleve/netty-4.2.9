# 10-01 连接生命周期与故障处理：超时 + 空闲检测 + 优雅关闭深度分析
...
> 📦 **可运行 Demo**：[Ch09_ConnectionLifecycleDemo.java](./Ch09_ConnectionLifecycleDemo.java) —— 连接生命周期事件验证，直接运行 `main` 方法即可。
...
### 2.4 ServerSocketChannel vs SocketChannel 状态转换对比（stateDiagram）

> 虽然两者共享相同的状态集合，但**触发条件和时机**不同，面试中经常考查区别。

```mermaid
stateDiagram-v2
    direction TB

    state "ServerSocketChannel 生命周期" as Server {
        [*] --> S_Created : new NioServerSocketChannel()
        S_Created --> S_Registered : register(bossEventLoop)<br/>fireChannelRegistered()
        S_Registered --> S_Active : bind(port) 成功<br/>fireChannelActive()<br/>→ beginRead() 注册 OP_ACCEPT
        S_Active --> S_Inactive : close()<br/>fireChannelInactive()
        S_Inactive --> S_Unregistered : deregister()<br/>fireChannelUnregistered()
        S_Unregistered --> [*]

        note right of S_Registered
            isActive() = isOpen() && socket.isBound()
            此时未 bind → isActive()=false
            channelActive 在 bind() 完成后触发
        end note
    }

    state "SocketChannel 生命周期" as Client {
        [*] --> C_Created : accept() 或 new NioSocketChannel()
        C_Created --> C_Registered : register(workerEventLoop)<br/>fireChannelRegistered()
        C_Registered --> C_Active : accept 的 Channel 已连接<br/>register 回调中 isActive()=true<br/>fireChannelActive()<br/>→ beginRead() 注册 OP_READ
        C_Active --> C_Inactive : close() / 对端关闭 / 异常<br/>fireChannelInactive()
        C_Inactive --> C_Unregistered : deregister()<br/>fireChannelUnregistered()
        C_Unregistered --> [*]

        note right of C_Registered
            isActive() = isOpen() && ch.isConnected()
            accept 出的 Channel 创建时已连接
            channelActive 在 register0() 回调中触发
        end note
    }
```
...
| 对比维度 | ServerSocketChannel | SocketChannel (accept 出的) |
|---------|--------------------|-----------------------------|
| **channelActive 触发位置** | `AbstractUnsafe.bind()` 完成后 | `AbstractUnsafe.register0()` 回调中 |
| **isActive() 判断** | `isOpen() && socket.isBound()` | `isOpen() && ch.isConnected()` |
| **beginRead 注册的事件** | `OP_ACCEPT` | `OP_READ` |
| **EventLoop 归属** | Boss EventLoop | Worker EventLoop |
| **关闭触发者** | 通常是主动调用 `serverChannel.close()` | 对端关闭 / 异常 / 主动调用 |

### 2.5 close() 内部子状态流程图

> Channel 的 close() 操作并非简单的 socket.close()，而是一个**多步骤的有序流程**。

**源码位置**：`AbstractChannel.AbstractUnsafe.close()` — `AbstractChannel.java` 第 548-625 行

```mermaid
flowchart TD
    Start(["close(promise) 入口"]) --> CheckCancel{"promise.setUncancellable()?"}
    CheckCancel -->|No| Return1(["return"])
    CheckCancel -->|Yes| CheckInited{"closeInitiated?"}

    CheckInited -->|Yes, 已在关闭中| CheckDone{"closeFuture.isDone()?"}
    CheckDone -->|Yes| SetSuccess["safeSetSuccess(promise)<br/>return"]
    CheckDone -->|No| AddListener["closeFuture.addListener(promise)<br/>return"]

    CheckInited -->|No, 首次关闭| Init["closeInitiated = true<br/>wasActive = isActive()"]
    Init --> NullOBB["outboundBuffer = null<br/>🔑 禁止后续 write/flush"]

    NullOBB --> CheckExecutor{"prepareToClose()<br/>返回 closeExecutor?"}

    CheckExecutor -->|"Yes (SO_LINGER > 0)"| LingerPath["closeExecutor.execute()"]
    LingerPath --> DoClose1["doClose() — 关闭 JDK Socket"]
    DoClose1 --> InvokeLater1["invokeLater → EventLoop"]
    InvokeLater1 --> FailFlushed1["outboundBuffer.failFlushed()<br/>通知所有 pending 写失败"]
    FailFlushed1 --> CloseOBB1["outboundBuffer.close()"]
    CloseOBB1 --> FireInactive1["fireChannelInactiveAndDeregister()"]

    CheckExecutor -->|"No (普通路径)"| DoClose2["doClose() — 关闭 JDK Socket"]
    DoClose2 --> FailFlushed2["outboundBuffer.failFlushed()"]
    FailFlushed2 --> CloseOBB2["outboundBuffer.close()"]
    CloseOBB2 --> CheckFlush{"inFlush0?<br/>当前是否在 flush 中?"}
    CheckFlush -->|Yes| Defer["invokeLater → 延迟执行<br/>fireChannelInactiveAndDeregister()"]
    CheckFlush -->|No| Direct["直接执行<br/>fireChannelInactiveAndDeregister()"]

    FireInactive1 --> FCI["fireChannelInactive()"]
    Defer --> FCI
    Direct --> FCI
    FCI --> FCU["fireChannelUnregistered()"]
    FCU --> End(["关闭完成"])

    style NullOBB fill:#ffcdd2
    style DoClose1 fill:#fff3e0
    style DoClose2 fill:#fff3e0
    style FCI fill:#e8f5e9
    style FCU fill:#e8f5e9
```
...
| 步骤 | 设计动机 |
|------|---------|
| `outboundBuffer = null` 🔑 | 立即阻止新的 write() 操作，write() 中检测到 null 会直接 fail |
| `prepareToClose()` | SO_LINGER 场景下 doClose() 会阻塞（等待数据发完），不能在 EventLoop 线程执行，需要 GlobalEventExecutor |
| `failFlushed()` | 将 ChannelOutboundBuffer 中已 flush 但未写完的 Entry 全部通知失败 |
| `outboundBuffer.close()` | 清理所有未 flush 的 Entry，释放 ByteBuf 引用计数 |
| `inFlush0` 检查 | 如果当前正在 flush 中（重入场景），延迟到当前 flush 完成后再触发 channelInactive，避免事件乱序 |
| 事件顺序保证 | **channelInactive → channelUnregistered**，严格按照此顺序（与建立时的 registered → active 相反） |

> ⚠️ **生产踩坑**：在 `channelInactive()` 回调中不要尝试 write()——此时 `outboundBuffer` 已经是 null，write 会直接失败。如果需要在连接关闭时发送最后的数据（如 WebSocket Close Frame），应该在 close 之前主动 writeAndFlush。
...
1. 核对记录：已对照 AbstractNioChannel.java 源码第 290-335 行，差异：无
2. 核对记录：已对照 AbstractNioChannel.java 源码第 380-400 行，差异：无
3. 核对记录：已对照 AbstractChannel.java 源码第 550-625 行，差异：无
4. 核对记录：已对照 AbstractChannel.java 源码第 627-640 行，差异：无
5. 核对记录：已对照 AbstractChannel.java 源码第 500-540 行，差异：无
6. 核对记录：已对照 IdleStateHandler.java 源码第 100-145 行，差异：无
7. 核对记录：已对照 IdleStateHandler.java 源码第 330-380 行，差异：无
8. 核对记录：已对照 IdleStateHandler.java 源码第 450-490 行，差异：无
9. 核对记录：已对照 IdleStateHandler.java 源码第 495-530 行，差异：无
10. 核对记录：已对照 IdleStateHandler.java 源码第 535-575 行，差异：无
11. 核对记录：已对照 IdleStateHandler.java 源码第 430-475 行，差异：无
12. 核对记录：已对照 IdleStateEvent.java 源码第 25-40 行，差异：无
13. 核对记录：已对照 ReadTimeoutHandler.java 源码第 65-104 行，差异：无
