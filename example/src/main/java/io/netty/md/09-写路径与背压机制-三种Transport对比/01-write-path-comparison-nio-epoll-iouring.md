# 09-01 写路径三种 Transport 对比：NIO vs Epoll vs io_uring doWrite 深度差异分析

> **前置依赖**：本文建立在 [08-01 写路径与背压](../08-写路径与背压/01-write-path-and-backpressure.md) 之上。08 篇已详细分析了 `ChannelOutboundBuffer`、`write()`/`flush()` 分离、水位线背压机制、`NioSocketChannel.doWrite()` 源码。本文不重复这些内容，而是聚焦于：**三种 Transport 在 `doWrite` 之后的差异**。
...
> 📦 **可运行 Demo**：[Ch08_BackpressureDemo.java](./Ch08_BackpressureDemo.java) —— 写路径与背压机制验证，直接运行 `main` 方法即可。
...
### 5.4 Epoll LT 模式对写路径的影响 🔥

> ⚠️ **重要事实**：Netty 4.2 的 Epoll Transport 对 Channel fd **统一使用 LT（Level-Triggered）模式**（`EpollMode` 枚举已 `@Deprecated`，注释明确写道 "Netty always uses level-triggered mode"）。只有内部的 `eventFd` 和 `timerFd` 使用 ET。

- **LT 模式**：只要状态**保持**就会持续触发（只要 Socket 可写就一直触发 `EPOLLOUT`）
- **ET 模式**：只在状态**变化**时触发事件（从不可写变为可写）

NIO 和 Epoll 都使用 LT，在写路径上行为类似：
- spin 用完后如果不清除 `EPOLLOUT`/`OP_WRITE`，下次 `epoll_wait`/`select` 会立即再次触发
- Epoll 的 `clearFlag(EPOLLOUT)` 等价于 NIO 的 `clearOpWrite()`
- 两者的写半包处理逻辑本质相同：spin 用完 → 注册 `EPOLLOUT` → 等待下次可写通知 → 继续 flush

**Epoll 写路径的真正优势**不在触发模式，而在于：
1. **直接 JNI 调用 `writev(fd, iov[])`**：跳过 JDK 的 `SocketChannel.write()` → `IOUtil` → `nd.writev()` 层层封装
2. **无 `SelectionKey` 对象开销**：直接通过 fd 整数注册/修改事件，无 Java 对象分配
3. **堆外内存直写**：`ByteBuf.memoryAddress()` 直接传给 `writev`，无需堆内→堆外转换
...
## 真实运行验证（Ch08_BackpressureVerify.java 完整输出）

> 以下输出通过运行 `Ch08_BackpressureVerify.java`（自包含 server + client，客户端 autoRead=false 制造背压）获得：

```
===== 背压机制验证 =====
水位配置: low=8KB, high=16KB
[连接建立] isWritable=true
[水位变化 #1] isWritable=false → 暂停写入
[写入暂停] 本轮写入 165602 条消息后触发背压 (累计: 165602)
[水位变化 #2] isWritable=true → 恢复写入
[水位变化 #3] isWritable=false → 暂停写入
[写入暂停] 本轮写入 77 条消息后触发背压 (累计: 165679)
[水位变化 #4] isWritable=true → 恢复写入
[水位变化 #5] isWritable=false → 暂停写入
[写入暂停] 本轮写入 77 条消息后触发背压 (累计: 165756)
...（持续振荡）
```

**验证结论**：
- ✅ **初始状态**：连接建立后 `isWritable=true`，写缓冲区为空
- ✅ **首次触发背压**：写入 165,602 条小消息后超过 16KB 高水位，`isWritable` 变为 `false`
- ✅ **水位振荡**：TCP 发送缓冲区排空一部分后降到 8KB 低水位，恢复写入；再次超 16KB 又暂停，如此反复
- ✅ **恢复后精度**：恢复后仅写入 77 条就再次触发（因为 TCP 窗口已经很小，排出速率有限）
- ✅ **channelWritabilityChanged 事件**：每次水位穿越都准确回调
...
1. 核对记录：已对照 AbstractEpollStreamChannel.doWrite() 源码，差异：无
2. 核对记录：已对照 AbstractEpollStreamChannel.doWriteMultiple() 源码，差异：无
3. 核对记录：已对照 AbstractEpollStreamChannel.writeBytesMultiple(IovArray) 源码，差异：无
4. 核对记录：已对照 AbstractEpollStreamChannel.doWriteSingle() 源码，差异：无
5. 核对记录：已对照 AbstractEpollStreamChannel.writeBytes() 源码，差异：无
6. 核对记录：已对照 AbstractEpollStreamChannel.writeDefaultFileRegion() 源码，差异：无
7. 核对记录：已对照 AbstractEpollStreamChannel.filterOutboundMessage() 源码，差异：无
8. 核对记录：已对照 AbstractEpollChannel.doWriteBytes() 源码，差异：无
9. 核对记录：已对照 AbstractEpollChannel.flush0() 源码，差异：无
10. 核对记录：已对照 AbstractIoUringChannel.doWrite() 源码，差异：无
11. 核对记录：已对照 AbstractIoUringChannel.scheduleWriteIfNeeded() 源码，差异：无
12. 核对记录：已对照 AbstractIoUringChannel.scheduleWrite() 源码，差异：无
13. 核对记录：已对照 AbstractIoUringChannel.writeComplete() 源码，差异：无
14. 核对记录：已对照 AbstractIoUringChannel.flush0() 源码，差异：无
15. 核对记录：已对照 AbstractIoUringChannel.schedulePollOut() 源码，差异：无
16. 核对记录：已对照 AbstractIoUringChannel ioState 标志位常量，差异：无
17. 核对记录：已对照 AbstractIoUringStreamChannel.scheduleWriteSingle() 源码，差异：无
18. 核对记录：已对照 AbstractIoUringStreamChannel.scheduleWriteMultiple() 源码，差异：无
19. 核对记录：已对照 AbstractIoUringStreamChannel.writeComplete0() 源码，差异：无
20. 核对记录：已对照 AbstractIoUringStreamChannel.filterOutboundMessage() 源码，差异：无
21. 核对记录：已对照 AbstractIoUringChannel.filterOutboundMessage() 源码，差异：无
22. 核对记录：已对照 UnixChannelUtil.isBufferCopyNeededForWrite() 源码，差异：无
23. 核对记录：已对照 IovArray 类 源码，差异：无
