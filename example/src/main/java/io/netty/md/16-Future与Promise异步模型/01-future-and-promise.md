...
> 📦 **可运行 Demo**：[Ch15_FuturePromiseDemo.java](./Ch15_FuturePromiseDemo.java) —— Future 与 Promise 异步模型验证，直接运行 `main` 方法即可。
...
## 真实运行验证（Ch15_FuturePromiseDemo.java 完整输出）

> 以下输出通过直接运行 `Ch15_FuturePromiseDemo.java` 的 `main` 方法获得（OpenJDK 11，Linux x86_64）：

```
===== 1. Promise 基本使用 =====
创建后: isDone=false isSuccess=false
完成后: isDone=true isSuccess=true result=Hello from Promise!

===== 2. Listener 异步回调 =====
[Listener] 成功! 结果=42 回调线程=multiThreadIoEventLoopGroup-2-1

===== 3. Promise 链式编排 =====
[链式] 步骤A完成: A的结果
[链式] 步骤B完成: B的结果（基于A: A的结果）

✅ Demo 结束
```

**验证结论**：
- ✅ `Promise` 创建后 `isDone=false`，`setSuccess()` 后 `isDone=true, isSuccess=true`
- ✅ Listener 回调在 EventLoop 线程中执行（`multiThreadIoEventLoopGroup-2-1`），而非调用线程
- ✅ Promise 链式编排：A 完成后触发 B，B 能拿到 A 的结果

---

## Ch15_FuturePromiseVerify 真实运行输出

> 📦 **验证代码**：[Ch15_FuturePromiseVerify.java](./Ch15_FuturePromiseVerify.java) — 直接运行 `main` 方法即可复现以下输出。

```
===== Ch15 Future/Promise 内部机制验证 =====

--- 验证 1：状态不可逆 ---
  初始: isDone=false isSuccess=false
  setSuccess 后: isDone=true isSuccess=true result=OK
  trySuccess 尝试修改: ✅ 返回 false（不可变）

--- 验证 2：重复 set 抛异常 ---
  ✅ 正确抛出: IllegalStateException: complete already: DefaultPromise@...(success: First)
  tryFailure 返回: ✅ false（已完成不可改）

--- 验证 3：Listener 回调线程 ---
  Listener 回调线程: multiThreadIoEventLoopGroup-2-1
  EventLoop 线程:    multiThreadIoEventLoopGroup-2-1
  是同一线程: ✅ 是

--- 验证 4：cancel() 语义 ---
  未完成时取消: cancel=true isCancelled=true
  已完成时取消: cancel=false isCancelled=false
  验证: ✅ 正确

--- 验证 5：sync() vs await() ---
  sync() 抛出异常: ✅ RuntimeException: test error
  await() 未抛异常: ✅ (await 不抛异常)
  但 isSuccess=false cause=test error 2
...
--- 验证 7：setUncancellable() ---
  setUncancellable(): true → cancel() 返回: false
  验证: ✅ 正确防取消，setSuccess 后正常完成

✅ Ch15 所有验证通过
```

> **🔍 验证结论**：
> - 状态不可逆 ✅：`setSuccess` 后 `trySuccess` 返回 false
> - 重复 set 抛异常 ✅：`IllegalStateException`
> - Listener 在绑定的 EventLoop 线程回调 ✅
> - cancel 语义 ✅：未完成可取消，已完成不可取消
> - sync() 抛异常，await() 不抛 ✅
> - Listener FIFO 顺序 ✅
> - setUncancellable 防取消 ✅
...
1. 核对记录：已对照 Future.java、Promise.java、DefaultPromise.java、ChannelFuture.java、ChannelPromise.java、DefaultChannelPromise.java、CompleteFuture.java、SucceededFuture.java、FailedFuture.java 源码，差异：无
2. 核对记录：已对照 DefaultPromise.java 第44-82行源码，差异：无
3. 核对记录：已对照 DefaultPromise.java isDone0/isSuccess/isCancelled0 方法源码，差异：无
4. 核对记录：已对照 DefaultPromise.java setSuccess0/setFailure0/setValue0/checkNotifyWaiters 方法源码，差异：无
5. 核对记录：已对照 DefaultPromise.java addListener/addListener0 方法源码，差异：无
6. 核对记录：已对照 DefaultPromise.java notifyListeners 方法源码，差异：无
7. 核对记录：已对照 DefaultPromise.java await/sync/rethrowIfFailed 方法源码，差异：无
8. 核对记录：已对照 DefaultPromise.java checkDeadLock 方法源码，差异：无
9. 核对记录：已对照 DefaultChannelPromise.java checkDeadLock 方法源码，差异：无
10. 核对记录：已对照 DefaultFutureListeners.java 源码，差异：无
11. 核对记录：已对照 DefaultFutureListeners.java add 方法源码，差异：无
12. 核对记录：已对照 DefaultPromise.java notifyListenersNow 方法源码，差异：无
13. 核对记录：已对照 PromiseCombiner.java 字段声明源码，差异：无
14. 核对记录：已对照 PromiseCombiner.java add/finish/operationComplete0/tryPromise 方法源码，差异：无
15. 核对记录：已对照 CompleteFuture.java 源码，差异：无
16. 核对记录：已对照 SucceededFuture.java、FailedFuture.java 源码，差异：无
17. 核对记录：已对照 ChannelFuture.java 源码，差异：无
18. 核对记录：已对照 DefaultChannelPromise.java 源码，差异：无
